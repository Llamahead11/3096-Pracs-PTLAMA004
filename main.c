...
int main(void){
  ...
  /* USER CODE BEGIN 1 */
	//debugPrintln(&huart2, "blarry\n");
	//debugPrintln(&huart2, decToBcd(5)+"\n");
	//debugPrintln(&huart2, bcdToDec(5)+"\n");
  ...
  //TASK 6
  //YOUR CODE HERE
  //setTime(0, 0, 12, 1, 20, 9, 2022-1970);
  ...
  while (1)
  {
	...
	//uint r = 0b0 + decToBcd(7);
	//sprintf(buffer, "%x \r", r);
	//debugPrintln(&huart2, buffer);

	getTime();
	uint current = epochFromTime(time);
	sprintf(buffer, "%d \r", current);
	debugPrintln(&huart2, buffer);

	int seconds = bcdToDec(time.seconds);
	int mins = bcdToDec(time.minutes);
	int hrs = bcdToDec(time.hour);

	sprintf(buffer, "%i :%i :%i",hrs,mins,seconds);
	debugPrintln(&huart2, buffer);
	pause_sec(1);

	uint32_t r = decToBcd(73);

	sprintf(buffer, "%d \r", r);
	debugPrintln(&huart2, buffer);

	sprintf(buffer, "%d \r", bcdToDec(r));
	debugPrintln(&huart2, buffer);
...
void pause_sec(float x)
{
	...
	//Make sure you've defined DELAY1 and DELAY2 in the private define section
	     ...
	     //DELAY 2 = 610
	//YOUR CODE HERE
	//runs for loop with run time = 1 second
...
void debugPrintln(UART_HandleTypeDef *uart_handle, char _out[]) {
	//function to print _out[] to uart
...
uint8_t decToBcd(int val)
{
	...
	//TO DO:
	//TASK 3
	return (  (val/10*16) + (val%10)  ); //returns BCD
	//YOUR CODE HERE
...
int bcdToDec(uint8_t val)
{
	...
	//YOUR CODE HERE
	return ((val/16*10)+(val%16));   //Returns decimal
...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	uint8_t set_time[7];             //declares array of length 7 with varying units of time

	set_time[0] = sec;               //first element is seconds
	time.seconds = set_time[0];      //sets RTC to time specified

	set_time[1] = min;               //second element is minutes
	time.minutes = set_time[1];

	set_time[2] = hour;              //third element is hours
	time.hour = set_time[2];

	set_time[3] = dow;               //fourth element is day of week
	time.dayofweek = set_time[3];

	set_time[4] = dom;               //5th element is day of month
	time.dayofmonth = set_time[4];

	set_time[5] = month;             //6th element is months
	time.month = set_time[5];

	set_time[6] = year;              //7th element is years
	...
	HAL_I2C_Mem_Write(&hi2c1, 0b11010000, 0x0, I2C_MEMADD_SIZE_8BIT, set_time, 7, 1000);
...
void getTime (void)
{
 //    Get the time from the RTC using I2C
	...
	uint8_t get_time[7];  // declare an array of length 7 of varying units of time
	...
	HAL_I2C_Mem_Read(&hi2c1, 0b11010001, 0x0, I2C_MEMADD_SIZE_8BIT, get_time, 7, 1000);


	//YOUR CODE HERE
	time.seconds = get_time[0];                //sets time struct to current time from RTC
...
int epochFromTime(TIME time){
	...
	//YOUR CODE HERE
	int seconds    = bcdToDec(time.seconds);           //gets values from struct time and converts from bcd to dec
	...
	int day = dayofmonth;                             //switch statement to get the number of days in month
	switch(month){
	case 1:
		day += 31;
	break;
	...
	int total = seconds + (minutes * 60) + (hour*60*60) + (day*60*60*24);
